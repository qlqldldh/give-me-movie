...
def test_rand_int_should_return_value_in_range_of_start_and_end(fake):
    start = fake.pyint()
    end = fake.pyint(min_value=start + 1)

    assert start <= rand_int(start, end) < end


def test_rand_word_should_return_value_in_list(fake):
